 ...
 *   - The mode file (~/.claude/harness/mode) is the ONLY switch. Missing or
 *     unrecognized → off → every hook is silent and creates nothing.
 *   - While on, the hooks' only possible output is the over-cap permission
 *     prompt. No nudges, no notes, no other speech.
 *   - Fail-open everywhere: malformed input → exit 0, empty or one JSON object.
 */
...
/**
 * Fresh state dir. `mode` null means "no mode file" (= off). `config`
 * undefined means "no harness.json"; an object writes one.
 */
function sandbox(mode = "on", config = undefined) {
  const home = mkdtempSync(join(tmpdir(), "harness-test-"));
  if (config !== undefined) writeFileSync(join(home, "harness.json"), typeof config === "string" ? config : JSON.stringify(config));
  if (mode !== null) {
...
check("no mode file → every hook silent, creates nothing (even with harness.json present)", () => {
  for (const h of ALL_HOOKS) {
    const home = track(sandbox(null, { caps: { fable: 3 } }));
    const r = run(h, h === "harness-core.mjs" ? { cwd: process.cwd() } : wf(repeat(50, "await agent('x',{model:'fable'})")), home);
    assert(r.raw.trim() === "", `${h} emitted output while off`);
    assert(r.code === 0, `${h} exited ${r.code}`);
    assert(!existsSync(join(home, "harness")), `${h} created state while off`);
  }
});

check("mode 'off' and junk modes ('standard', nonsense, oversized) → silent", () => {
  for (const m of ["off", "standard", "nonsense", "x".repeat(200)]) {
    for (const h of ALL_HOOKS) {
      const home = track(sandbox(m));
      const r = run(h, h === "harness-core.mjs" ? { cwd: process.cwd() } : wf(repeat(50, "await agent('x',{model:'fable'})")), home);
      assert(r.raw.trim() === "", `${h} spoke in mode ${JSON.stringify(m.slice(0, 12))}`);
    }
  }
});

check("legacy enabled:false in harness.json is ignored — the mode file rules", () => {
  const home = track(sandbox("on", { enabled: false }));
  const r = run("harness-workflow.mjs", wf(repeat(20, "await agent('t',{model:'opus'})")), home);
  assert(decision(r) === "ask", `guardrail must stay up despite legacy enabled:false, got ${decision(r)}`);
});

check("corrupt harness.json → defaults, guardrail stays up", () => {
  const home = track(sandbox("on", "{not json"));
  const r = run("harness-workflow.mjs", wf(repeat(4, "await agent('t',{model:'fable'})")), home);
  assert(decision(r) === "ask", `corrupt overrides must fall back to default caps, got ${decision(r)}`);
});

check("caps override in harness.json applies", () => {
  const home = track(sandbox("on", { caps: { fable: 1 } }));
  let r = run("harness-dispatch.mjs", task("fable"), home);
  assert(r.raw.trim() === "", `1st fable under a cap of 1 should be silent`);
  r = run("harness-dispatch.mjs", task("fable"), home);
  assert(decision(r) === "ask", `2nd fable should ask with caps.fable=1, got ${decision(r)}`);
});
...
check("core: mode 'on' injects reuse + routing, not the agents posture", () => {
  const c = ctx(run("harness-core.mjs", { cwd: process.cwd() }, track(sandbox("on"))));
  assert(/ListAgents/.test(c) && /sonnet/.test(c), `on-block missing reuse/routing: ${c.slice(0, 120)}`);
  assert(!/AGENTS MODE/.test(c), "agents posture leaked into 'on'");
});

check("core: mode 'agents' injects the orchestration posture, self-contained", () => {
  const c = ctx(run("harness-core.mjs", { cwd: process.cwd() }, track(sandbox("agents"))));
  assert(/AGENTS MODE/.test(c), `expected agents posture: ${c.slice(0, 120)}`);
  assert(/END your turn/i.test(c), `agents block must carry the never-block rule: ${c.slice(0, 120)}`);
  assert(/ListAgents/.test(c), "agents block must be self-contained (reuse rule missing)");
});

check("core: personal ~/.claude/harness-core.md override wins", () => {
  const home = track(sandbox("on"));
  writeFileSync(join(home, "harness-core.md"), "## on\n```\nMY PERSONAL CORE\n```\n");
  const c = ctx(run("harness-core.mjs", { cwd: process.cwd() }, home));
  assert(/MY PERSONAL CORE/.test(c), `override ignored: ${c.slice(0, 80)}`);
});

// -------------------------------------------------- workflow fan-out caps ---

check("workflow: under every cap → completely silent", () => {
  const home = track(sandbox("on"));
...
check("workflow: over the opus cap (>15) → ASK, reason names the count", () => {
  const r = run("harness-workflow.mjs", wf(repeat(20, "await agent('t',{model:'opus'})")), track(sandbox("on")));
...
check("workflow: over the fable cap (>3) → ASK", () => {
  const r = run("harness-workflow.mjs", wf(repeat(4, "await agent('t',{model:'fable'})")), track(sandbox("on")));
  assert(decision(r) === "ask" && /4 fable \(cap 3\)/.test(reason(r)), `got ${decision(r)}: ${reason(r)}`);
});

check("workflow: over the sonnet cap (>30) → ASK", () => {
  const r = run("harness-workflow.mjs", wf(repeat(31, "await agent('t',{model:'sonnet'})")), track(sandbox("on")));
  assert(decision(r) === "ask" && /31 sonnet \(cap 30\)/.test(reason(r)), `got ${decision(r)}: ${reason(r)}`);
});
...
check("workflow: unannotated fan-out on a fable session counts as fable → ASK", () => {
  ...
  assert(decision(r) === "ask", `expected ask, got ${decision(r)}`);
  assert(/no model/.test(reason(r)) && /fable/.test(reason(r)), `should explain the inherited model: ${reason(r)}`);
});
...
check("workflow: asks but never denies or allows", () => {
  const r = run("harness-workflow.mjs", wf(repeat(50, "await agent('t',{model:'fable'})")), track(sandbox("on")));
  assert(decision(r) === "ask", `flagged fan-out must ask, got ${decision(r)}`);
});

check("workflow: 15k sites completes and stays fast", () => {
  const started = Date.now();
  const r = run("harness-workflow.mjs", wf(repeat(15000, "await agent('t',{model:'opus'})")), track(sandbox("on")));
...
check("workflow: a saved workflow with no script isn't policed", () => {
  const r = run("harness-workflow.mjs", { tool_name: "Workflow", cwd: process.cwd(), tool_input: { name: "saved" } }, track(sandbox("on")));
...
check("dispatch: a single worker of any tier is silent", () => {
  for (const model of ["fable", "opus", "sonnet", "haiku"]) {
    const r = run("harness-dispatch.mjs", task(model), track(sandbox("on")));
...
check("dispatch: the 4th fable in a window → ASK", () => {
  ...
  assert(/4 fable/.test(reason(last)) && /cap 3/.test(reason(last)), `reason: ${reason(last)}`);
});
...
check("dispatch: haiku is uncapped — a 40-worker burst stays silent", () => {
  const home = track(sandbox("on"));
  let r;
  for (let i = 0; i < 40; i++) r = run("harness-dispatch.mjs", task("haiku"), home);
  assert(r.raw.trim() === "", `haiku must never prompt: ${reason(r)}`);
});

check("dispatch: a stale window resets the count", () => {
  const home = track(sandbox("on", { capWindowSeconds: 1 }));
  for (let i = 0; i < 3; i++) run("harness-dispatch.mjs", task("fable"), home);
  execFileSync("node", ["-e", "setTimeout(()=>{}, 1200)"]); // wait out the 1s window
...
check("dispatch: no model on a fable session → SILENT (no nudge), but counted — 4th asks", () => {
  const home = track(sandbox("on"));
  const t = transcript(home, "claude-fable-5");
  let r = run("harness-dispatch.mjs", task(undefined, { transcript_path: t }), home);
  assert(r.raw.trim() === "", `an under-cap dispatch must be completely silent, got: ${ctx(r) || reason(r)}`);
  let last;
  for (let i = 0; i < 3; i++) last = run("harness-dispatch.mjs", task(undefined, { transcript_path: t }), home);
  assert(decision(last) === "ask", `unannotated fable workers must accumulate to the cap, got ${decision(last)}`);
});

check("dispatch: no model + sonnet session is silent", () => {
  const home = track(sandbox("on"));
  const t = transcript(home, "claude-sonnet-5");
...
check("state dir with spaces and non-ASCII resolves", () => {
  ...
  mkdirSync(join(home, "harness"), { recursive: true });
  writeFileSync(join(home, "harness", "mode"), "on");
  const r = run("harness-workflow.mjs", wf(repeat(20, "await agent('t',{model:'opus'})")), home);
